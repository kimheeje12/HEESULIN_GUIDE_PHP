...
<body>
    ...
    $bno=$_GET['image_id'];
    $sql="SELECT * FROM people_board WHERE image_id='".$bno."'";
    $result = mysqli_query($connect, $sql);
    $board = $result->fetch_array();  


    //이미지
    $sql2="SELECT * FROM image2 WHERE image_id='".$bno."'";
    ...
    <form class="maintext" form method="POST" action="people_modify_ok.php?image_id=<?php echo $bno;?>" enctype="multipart/form-data">
        <h1 style="font-size: 50px; margin-top: 30%;"> People Upload</h1>
        <h1>Name</h1>
        <p><textarea name="title" cols=20 rows=2 placeholder="Name" style= "font-size: 25px; text-align:center;"><?php echo $board["people_name"]?></textarea></p>
        <h1>Basic Info</h1>
        <p><textarea name="info" cols=25 rows=6 placeholder="Basic Info" style= "font-size: 25px; text-align:center;"><?php echo $board["people_info"]?></textarea></p>
        <h1>Opinion</h1>
        <textarea name="opinion" cols=30 rows=8 placeholder="Opinion" style= "font-size: 25px; text-align:center;"><?php echo $board["people_opinion"]?></textarea>
