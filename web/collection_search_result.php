...
<body>
    ...
    if(isset($_GET['page'])){
        ...
        $sql = mq("SELECT * FROM collection_board where collection_title like '%$collection_search%'");
    ...
    $sql3 = mq("SELECT * FROM collection_board where collection_title like '%$collection_search%' order by image_id asc limit $start_num, $list");
    while($board = $sql3->fetch_array()){
        $image=$board["image_id"];
   
    $query = "SELECT * FROM image where image_id = $image";  
    ...
    <div id="page_num">
            <ul id="page">
                ...
                if($page <= 1)
                {
                    ...
                    echo"<li><a href='?search=$collection_search&page=1'>처음</a></li>";
                }
                if($page <= 1)
                {
                    ...
                    $pre = $page - 1;
                    echo"<li><a href='?search=$collection_search&page=$pre'>이전</a></li>";
                }
                for($i=$block_start; $i<=$block_end; $i++){
                    if($page == $i){
                        echo"<li class='fo_re'>[$i]</li>";
                    }else{
                        echo"<li><a href='?search=$collection_search&page=$i'>[$i]</a></li>";
                    }
                }
                if($block_num >= $total_block){
                }else{
                    $next = $page + 1;
                    echo "<li><a href='?search=$collection_search&page=$next'>다음</a></li>";
                }
                if($page >= $total_page){
                    echo"<li>마지막</li>";
                }else{
                    echo"<li><a href='?search=$collection_search&page=$total_page'>마지막</a></li>";
                }
                ?>
            </ul>
        </div>
